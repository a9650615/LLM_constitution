...
鏡像 en v2.14（2026-07-09）。權威版：`docs/40-maintenance.md`（英文），衝突以英文版為準。
...
- `archive/` 只准追加、不改寫的「有日期歷史」：創始中文筆記
  （`00-diagnosis.md`、`90-letter.md`；交接註記，不翻譯）**以及**有日期的
  逐機器 bindings 快照（例：`BINDINGS-founding-machine.md`）。時代性機器事實
  可以留在那裡當「有日期的歷史」——絕不放進 `docs/`，也絕不當現行事實
  （現行事實＝該機器自己生成的 `BINDINGS.md`）。
