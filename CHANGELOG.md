...
## [2.7.3] — 2026-07-09
- BINDINGS templatized (published-plugin fix): the repo now ships
  `BINDINGS.template.md`; the real `BINDINGS.md` is per-machine, generated by
  `/rebind`, and gitignored — never committed again. The founding machine's
  instance is preserved at `archive/BINDINGS-founding-machine.md` (moved, with
  an appended relocation note; original content untouched, per docs/40 §6
  append-only). Reason: a published plugin must not distribute one machine's
  facts, nor an authorization artifact (§Standing exemptions' push
  pre-authorization), to every installer. `commands/rebind.md`, `README.md`,
  `INSTALL.md` updated to generate-from-template; docs/40 v2.14 (mirrored in
  zh/40) generalizes the archive bullet to cover dated bindings snapshots.
  **Migration note for existing checkouts**: if you have a tracked
  `BINDINGS.md` from before this version, it is now gitignored and will show
  as untracked/ignored — no data is deleted, but git will stop tracking
  further edits to it; run `/rebind` to confirm it's current, or diff it
  against `BINDINGS.template.md` if unsure.
