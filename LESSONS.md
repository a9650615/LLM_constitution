...
## L9. Raising a model-cache limit converts eviction thrash into OOM — derive the cap from the memory budget, and re-check the box after raising it (2026-07-11)
- Situation: Lemonade's LLM slot limit was raised 2→8 to stop two models evicting each other (a compaction-loop fix). Hours later the box OOMed: with 8 slots nothing ever evicts, on-demand loads accumulated to 5 resident LLMs (60 GiB GTT), and with ComfyUI alongside RAM hit 121/121 + swap 15/15; the kernel OOM killer started shooting processes.
- Wrong way: picking a "generous" limit to make eviction problems disappear. Eviction pressure and OOM are the same budget seen from opposite ends — removing one silently hands the overflow to the other.
- Right way: (1) enumerate what must coexist (here: pinned NPU model + main 35B + compact E2B + one floating advisor slot = 4) and cap exactly there, letting the floater LRU-evict (~5-12s reload — cheap vs OOM); (2) immediate relief without a service restart: `POST /api/v0/unload {"model_name": ...}` on idle models (freed 15 GiB GTT / 23 GiB RAM live); (3) after ANY capacity-limit raise, watch `free -g`, swap si/so, and GTT for one real workload cycle before declaring it done — the failure showed up only under the user's real ComfyUI + agent load, not in the fix's own verification.
